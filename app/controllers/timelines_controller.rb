class TimelinesController < ApplicationController
  ...
  def create
    @timeline = Timeline.new(timeline_params.merge(user_id: Current.user.id))
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to timelines_path, status: :see_other, notice: "Timeline was successfully destroyed." }
  ...
  def timeline_params
    permitted = params.require(:timeline).permit(:title, :info, :events)
    if permitted[:events].is_a?(String)
